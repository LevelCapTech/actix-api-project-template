...
fn main() {
    ...
    match matches.value_of("operation") {
        Some("GetIndex") => {
            let result = rt.block_on(client.get_index(
            ));
        ...
        Some("ItemsPost") => {
            let result = rt.block_on(client.items_post(
                  ???
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        */
        Some("ItemsIdDelete") => {
            let result = rt.block_on(client.items_id_delete(
                  56
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        Some("ItemsIdGet") => {
            let result = rt.block_on(client.items_id_get(
                  56
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        /* Disabled because there's no example.
        Some("ItemsIdPut") => {
            let result = rt.block_on(client.items_id_put(
                  56,
                  ???
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
